...
class Github:
    ...
    def _send_request(self, url):
        ...
        while tries <= 3:
            ...
            if resp.status_code != 200:
                self._log.error(
                    "Problem with getting data via url %s. Code: %s, Error: %s", url, resp.status_code, resp.text
                )
                ...
                if resp.status_code == 422:
                    ...
                    if r:
                        if "errors" in r.keys():
                            for e in r["errors"]:
                                if (
                                    e["resource"] == "PullRequest"
                                    and e["field"] == "diff"
                                    and e["code"] == "not_available"
                                    and "per_page" in url
                                ):  # we try to be as explicit as possible here
                                    self._log.error("unfetchable files for pull request, returning [], try: %s", tries)
                                    return []
                if (
                    resp.status_code == 500 and tries == 2
                ):  # problem for some part of endpoints, e.g., https://api.github.com/repos/apache/kafka/pulls/3490/reviews/48104142/comments?&page=1&per_page=100
                    self._log.error(
                        "Problem with getting data via url %s. Code: %s, Error: %s", url, resp.status_code, resp.text
                    )
                    if "per_page" in url:
    ...
    def _get_commit_id(self, revision_hash, repo_url):
        ...
        for vcs in VCSSystem.objects.filter(project_id=self._p.id):
            ...
            try:
                ...
                Commit.DoesNotExist
            ):  # happens for deleted branches of force pushes, e.g. https://github.com/ravibpatel/AutoUpdater.NET/commit/8dd52654733d688b0111064fd1a841f6769dc082
                pass
        return commit_id
    ...
    def run(self):
        ...
        self.pr_system = PullRequestSystem(
            project_id=self._p.id, url=self.config.tracking_url, collection_date=datetime.datetime.now()
        )
        self.pr_system.save()
    ...
    def fetch_pr_list(self):
        ...
        url = "{}?state=all".format(self.config.tracking_url)
        return self._fetch_all_pages(url)
    ...
    def parse_pr_list(self, prs):
        ...
        for pr in prs:
            ...
            try:
                mongo_pr = PullRequest.objects.get(
                    pull_request_system_ids=self.last_system_id, external_id=str(pr["number"])
                )
